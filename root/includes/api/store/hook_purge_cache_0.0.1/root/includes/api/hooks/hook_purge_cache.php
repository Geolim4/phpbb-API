...
* @version $Id: hook_purge_cache.php v0.0.1 13h13 10/19/2013 Geolim4 Exp $
...
namespace phpbb_api\hooks\methods
{
	...
	if (!defined(__NAMESPACE__ . '\PURGE_HOOK_API_TARGET_VERSION') )
	{
		...
		function hook_api_purge($mom, $data, $type)
		{
			$mom->require_admin_privilege(true, substr(\phpbb_api\rmvnmspce(__FUNCTION__), 5));
			...
			cache_moderators();
			\phpbb_api\api_add_log('API_LOG_PURGE_CACHE', $mom->api_key);
	...
	***/
		...
		function hook_api_sanitize_purge($mom, &$data, &$type)
		{
