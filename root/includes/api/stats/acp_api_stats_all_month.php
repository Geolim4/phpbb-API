...
^>@version $Id: acp_api_stats_single_month.php v0.0.1 00h11 12/20/2013 Geolim4 Exp $
...
$range_month = array_slice(explode(',', request_var('range_month', date('M'))), 0, 12, true);

$seed = preg_replace("/[^a-z0-9]+/", "", request_var('seed', ''));
$queries_history = $ip_history = array();
$months_label = phpbb_api\functions\generate_yearly_months($range_year);
$requests_per_month = $ip_per_month = array_fill_keys(array_flip($months_label), 0);
$hightest_score = $average = $final_score = 0;
$where_sql = '';
$begin_time = (int) mktime(0, 0, 0, min(phpbb_api\functions\inttostrtime($range_month, 'M', true)) - 1, 32, $range_year);
date('I', $begin_time) ? $begin_time = ($begin_time + API_HOUR_SECONDS): false;
$end_time = (int) mktime(0, 0, 0, max(phpbb_api\functions\inttostrtime($range_month, 'M', true)) + 1, 32, $range_year);
if (!empty($config['api_mod_cache_stats']))
{
	$cached_stats = $cache->get("_{$range_year}_" . str_replace('.' . $phpEx, '', substr(strrchr(__FILE__, DIRECTORY_SEPARATOR), 1)));
	if ($cached_stats !== false)
	{
		list($requests_per_month, $ip_per_month, $months_label, $hightest_score, $final_score) = unserialize($cached_stats);
		goto skip_sql_loop;
	}
}
...
foreach ($queries_history AS $queries_history_)
{
	...
	if ($year == $range_year && in_array($month, $range_month))
	{
		//Attach year each January ;)
		$months_label[$month . $year] = (($month == 'Jan') ? $year . '  ' . $month : $month);
		if (isset($requests_per_month[$month . $year]))
		{
			$requests_per_month[$month . $year]++;
		}
		else
		{
			$requests_per_month[$month . $year] = 1;
		}
		if (!isset($ip_history[$month . $year]))
		{
			$ip_history[$month . $year] = array();
		}
		if (!isset($ip_history[$month . $year][$queries_history_['ip']]))
		{
			$ip_history[$month . $year][$queries_history_['ip']] = 1;
			$ip_per_month[$month . $year]++;
		}
		if ($hightest_score < $requests_per_month[$month . $year])
		{
			$hightest_score = $requests_per_month[$month . $year];
		}
		$average++;
	}
}
$final_score = $average / sizeof($requests_per_month);
if (array_sum($requests_per_month) == 0 /*&& empty($hour_label)*/)
{
	$no_result = true;
	$requests_per_month[] = VOID;
}
if (!empty($config['api_mod_cache_stats']))
{
	$cache->put("_{$range_year}_" . str_replace('.' . $phpEx, '', substr(strrchr(__FILE__, DIRECTORY_SEPARATOR), 1)), serialize(array($requests_per_month, $ip_per_month, $months_label, $hightest_score, $final_score)), 7200);
	skip_sql_loop:
}

/* Create and populate the pData object */
$bertie_colorize_the_world = new pData();  
$bertie_colorize_the_world->addPoints(array_slice($requests_per_month, 0, 60, true), $user->lang['ACP_PHPBB_API_STATS_ALL_QR']);
$bertie_colorize_the_world->addPoints(array_slice($ip_per_month, 0, 60, true), $user->lang['ACP_PHPBB_API_STATS_YEAR_IP']);
$bertie_colorize_the_world->setSerieTicks($user->lang['ACP_PHPBB_API_STATS_ALL_QR'], 4);
$bertie_colorize_the_world->setAxisName(0, $user->lang['ACP_PHPBB_API_STATS_YEAR_REQ']);
$bertie_colorize_the_world->addPoints(phpbb_api\functions\phpbb_datify(array_slice($months_label, 0, 60, true)),"Labels");
...
$bertie_picture->drawText(160,49,$user->lang['ACP_PHPBB_API_STATS_YEAR'] . ": " . $range_year . '   ' . $user->lang('ACP_PHPBB_API_STATS_TOTAL', floor(array_sum($requests_per_month))),array("FontSize" => 16,"Align" => TEXT_ALIGN_BOTTOMLEFT));
