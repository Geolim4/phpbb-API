...
^>@version $Id: acp_api_stats_single_hour.php v0.0.1 00h11 12/20/2013 Geolim4 Exp $
...
$range_day = request_var('range_day', (int) date('d'));//There is one day only as in hour mode

$seed = preg_replace("/[^a-z0-9]+/", "", request_var('seed', ''));
$years_label = $queries_history = $ip_history = array();
$hour_label = phpbb_api\functions\generate_daily_hours();
...
//Not really needed as per hour statistics, also it will fill too fast your cache directory, uncomment it at your own risk !!
/*if (!empty($config['api_mod_cache_stats']))
{ 
	$cached_stats = $cache->get("_{$range_year}{$range_month}{$range_day}_" . str_replace('.' . $phpEx, '', substr(strrchr(__FILE__, DIRECTORY_SEPARATOR), 1)));
	if ($cached_stats !== false)
	{
		list($requests_per_hour, $ip_per_hour, $hour_label, $hightest_score, $final_score) = unserialize($cached_stats);
		goto skip_sql_loop;
	} 
}*/
...
foreach ($queries_history AS $queries_history_)
{
	...
	if ($year == $range_year && $range_month == $month && $dayint == $range_day)
	{
		$daystr = date('D', $queries_history_['time']);
		$hour =  $user->format_date($queries_history_['time'], $user->lang['ACP_PHPBB_API_STATS_HOUR']);

		$hour_label[$hour] = $hour;
		if (isset($requests_per_hour[$hour]))
		{
		...
		if (!isset($ip_history[$hour]))
		{
			$ip_history[$hour] = array();
		}
		if (!isset($ip_history[$hour][$queries_history_['ip']]))
		{
			$ip_history[$hour][$queries_history_['ip']] = 1;
			$ip_per_hour[$hour]++;
		}
		if ($hightest_score < $requests_per_hour[$hour])
		{
			$hightest_score = $requests_per_hour[$hour];
		}
		$average++;
	}
}
phpbb_api\functions\dsort($hour_label, 5);
phpbb_api\functions\dsort($requests_per_hour, 5);
$final_score = $average / sizeof($requests_per_hour);
if (array_sum($requests_per_hour) == 0 /*&& empty($hour_label)*/)
{
	$no_result = true;
	$requests_per_hour[] = VOID;
}

/*if (!empty($config['api_mod_cache_stats']))
{
	$cache->put("_{$range_year}{$range_month}{$range_day}_" . str_replace('.' . $phpEx, '', substr(strrchr(__FILE__, DIRECTORY_SEPARATOR), 1)), serialize(array($requests_per_day, $ip_per_day, $day_label, $hightest_score, $final_score)), 86400);
	skip_sql_loop: 
}*/

/* Create and populate the pData object */
$bertie_colorize_the_world = new pData();  
$bertie_colorize_the_world->addPoints(array_slice($requests_per_hour, 0, 60, true), $user->lang['ACP_PHPBB_API_STATS_ALL_QR']);
$bertie_colorize_the_world->addPoints(array_slice($ip_per_hour, 0, 60, true), $user->lang['ACP_PHPBB_API_STATS_DAY_IP']);
$bertie_colorize_the_world->setSerieTicks($user->lang['ACP_PHPBB_API_STATS_ALL_QR'], 4);
$bertie_colorize_the_world->setAxisName(0, $user->lang['ACP_PHPBB_API_STATS_DAY_REQ']);
$bertie_colorize_the_world->addPoints(phpbb_api\functions\phpbb_datify(array_slice($hour_label, 0, 60, true)),"Labels");
