 /*
     ...
     phpBB bridge: v0.0.1 00h11 12/20/2013 Geolim4 Exp $
...
if(document.all)
 ...
 function addArea(imageMapID,shapeType,coordsList,actionOver,actionOut, ImageMapToURL)
  {
   ...
   for(i=0;i<=Zones.length-2;i++)
    {
     ...
     addArea(ImageMapID,Options[0],Options[1],'showDiv("testDiv","'+Options[2]+'","'+Options[3]+'","'+Options[4].replace('"','')+'");','hideDiv("testDiv");', (typeof(map_uri[ImageMapID]) != 'undefined' ? map_uri[ImageMapID][i+1] : '' ));
    }
    $('#day-stats-img, img.lazy').maphilight();
	$('#day-stats-img, #seed_' + ImageMapID.replace('pictureMap-', '')).prevAll('canvas').effect('bounce');
  }
